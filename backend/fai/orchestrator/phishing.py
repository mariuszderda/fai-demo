...
class PhishingPlaybook(Playbook):
    """End-to-end phishing incident playbook."""

    name = "phishing"

    def _sender_domain(self, iocs: list[IoC], fallback: str | None) -> str:
        """Pick the most relevant sender domain from accepted IoCs."""
        for ioc in iocs:
            if ioc.status == IocStatus.ACCEPTED and ioc.type.value in {"domain", "url"}:
                return ioc.value
        return fallback or "unknown-domain"
    ...
    async def run(self, incident: Incident) -> None:
        ...
        self.incident_store.set_iocs(incident.id, all_iocs)
        malicious_count = len(
            [ioc for ioc in accepted_iocs if ioc.reputation == Reputation.MALICIOUS]
        )
        await self.event_bus.publish(
            incident.id,
            "ti_lookup_completed",
        ...
        malicious_domain_or_url = any(
            ioc.reputation == Reputation.MALICIOUS and ioc.type.value in {"domain", "url"}
            for ioc in accepted_iocs
        )
        severity = (
            Severity.HIGH
            if malicious_domain_or_url and "T1566" in techniques
            else Severity.MEDIUM
        )
        self.incident_store.update(incident.id, severity=severity)

        if severity == Severity.HIGH:
            sender_domain = self._sender_domain(accepted_iocs, incident.host_id)
            reason = f"Phishing confirmed: quarantine mail relay for {sender_domain}"
            approval = await self.approval_gate.request(
                incident.id,
                host_id=sender_domain,
                reason=reason,
                isolation_target="mail_relay_quarantine",
            )
            self.incident_store.update(
                incident.id,
            ...
            if approved.decision == ApprovalDecision.APPROVED and approved.isolation_token:
                case_response = await self.siem_consumer.client.post(
                    "/mock/soar/case",
                    json={
                        "title": f"Phishing incident {incident.id}",
